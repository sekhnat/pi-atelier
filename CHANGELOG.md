...
- Add an opt-in composer session ribbon (`showSessionRibbon`, default off, user configuration only): the composer's top border renders activity, model/thinking, workspace/Git, and context with Nerd Font prompt icons, and a compact row below shows measured usage, cache, cost, and response timing. The complete plain Status Rail remains the default and renders whenever the ribbon cannot (no composer editor, a Pi selector, fewer than 12 terminal rows, or an editor too narrow for the strip). Enabling the ribbon requires a Nerd Font selected in the terminal; Atelier neither installs fonts nor changes terminal settings.
- Keep inline transcript images visible beside Sidebar panels in regular and fullscreen mode: graphics commands are preserved byte-for-byte and redrawn at their original columns after text compositing, fullscreen Sidebar cells are repaired on image rows, and visible images temporarily hide (with their reserved layout space retained) while a capturing overlay is open so dialogs stay readable.
- Exclude Sidebar text from fullscreen screen selection and OSC 52 copies when a mouse drag starts outside the transcript, including drags that begin in the editor or the Sidebar; transcript, capturing-overlay, resize, and regular-mode selection behavior is unchanged.
- Suspend expensive producers while Atelier is disabled: sidebar/footer renders, usage-history scans, TODO reconstruction, streaming token estimates, Workspace Pulse scheduling, and completion notifications stop, pending and in-flight Git inspections are cancelled, and partially observed response timing is discarded. `/atelier enable` reconciles once from current session state without reopening the Sidebar.
- Skip redundant HEAD and diff commands in Workspace Pulse when repository status reports no tracked changes, keeping branch identity and untracked-file counts.
- Fit Sidebar content by measuring panel rows and shared panel chrome instead of repeatedly painting discarded candidate panels, preserving panel order, defaults, and drop priorities.
- Make Display Settings Undo restore only the most recent Display or Sidebar mutation: a Display edit no longer falls back to an older Sidebar edit (and vice versa), a successful Sidebar save clears only a Sidebar undo record, and a failed save preserves the pending undo target.
